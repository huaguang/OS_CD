...
int main(int argc,char* argv[]){
	...
	int comBegPos[20];	//commands begin position in arguments when pipe '|' is used;
	char **args;		//arguments;
	short argNum=0;		//arguments' number
	pid_t child_id;		
	int redirectPos=0;	//where is the '>'  and '0' means no '>' in this command
	char*redirectedFileName;	
	int fd[2];			//when pipe '|' used, this passes in and out for two command
	int readPipe;		//when pipe '|' used, this store read port of the fd
	int comNum=0;		//commands' number.when pipe '|' used,there will be more than one command in arguments
	int lastOut[2];		//when '>' used,this lastOut[0] will be used for writing to file 
						//and lastout[1] used to store the last but one process' output
	getCurPath(curPath);//init the curPath;
	printf("%s$",curPath);// prefix for the prompt
	gets(command);		
...
//executive a command by Using system command in another process;
